...
label guide_naming_start:
    ...
    if persistent.name_mc:
        ...
        guide "不知道为什么，{w=0.5}直呼别人的名字会让我有种奇怪的羞耻感。"
        guide "我想你应该知道自己叫什么吧，{w=0.5}对吧？"
        ...
        menu:
            ...
            "我还是换一个吧。":
                $ duplicate_name_attempts = 0
                guide "好吧，{w=0.5}换个身份也是个很好的选择。{w=0.25}稍等一下，{w=0.25}谢谢。\n{w=1}{color=#cccccc}（我把取名界面丢到哪去了{......}）"
                jump guide_naming_loop

    guide "呃，{w=0.25}所以，{cps=*0.9}{w=0.25}我应该叫你{......}{w=0.5}{cps=*0.9}玩家？"
    guide "{cps=*0.9}行吧，{w=0.5}好吧，{w=0.5}你好玩家，{w=0.5}玩家你好，{w=0.5}嗯。"
    guide "那么首先，{w=0.25}呃，{w=0.25}谢谢你愿意游玩我们的作品，{w=0.25}我想是的。"
    stop music fadeout 2.0
    guide "然后，{w=1}{cps=*0.8}呃{......}{w=2}{nw}"
    ...
    # 如果在显示时间内输入了内容：
    if entered == "Entered":
        guide "哇，{w=0.25}你的速度可真快，只用了[delta_time:.3f]秒{......}\n你大概只是乱敲了下键盘吧，{w=0.25}我猜。"
...
label guide_naming_entered:
    ...
    if not player_input:
        ...
        if empty_name_attempts == 1:
            guide "{......}奇怪，我的程序没对这种情况做出限制吗？{w=1.0}开发者还真是粗心。"
            guide "算了，{w=0.25}将就着用吧，{w=0.25}反正一个存档也只用一次。"
            jump guide_naming_loop

        elif empty_name_attempts == 2:
            guide "你是故意的吗？{w=0.25}我可不觉得这很好玩。"
            guide "抱歉，{w=0.25}我不是在威胁你，{w=0.25}不过我们还是别在取名界面耽搁太久吧。"
            jump guide_naming_loop
            ...
            guide "嗯，{w=0.25}你觉得，{w=0.5}“XDDCC”这个名字怎么样？{w=0.25}{nw}"
            guide "啊哈哈，{w=0.25}抱歉，{w=0.25}我只是开个玩笑，{w=0.25}你可能不知道这个梗，{w=0.5}实在是不好意思。"
            guide "我可没有什么随机名字生成器，{w=0.25}不过这个名字绝对不能用。"
            guide "{w=0.25}我可不想惹上官司，{w=0.5}更不用说这名字简直烂透了。"
            guide "看来还得我自己想一个{......}{nw}"
...
label guide_naming_done:
    ...
    guide "呃，{w=0.5}你还有其他疑问吗？{w=2}{nw}"
    guide "哦不对，{w=0.25}你又问不了我。（笑）"
    guide "好吧，稍等一下，{w=0.5}我得找找剧情被我放在哪了{......}{nw}"
    stop music fadeout 2.0
